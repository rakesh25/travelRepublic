enyo.kind({
	...
	components: [
		{name: 'list', kind: 'AroundList', classes: 'list-sample-around', fit: true, multiSelect: true, onSetupItem: 'setupItem', aboveComponents: [
			{kind: 'moon.Toolbar', layoutKind: 'FittableColumnsLayout', components: [
				{kind: 'moon.InputDecorator', fit: true, noStretch: true, layoutKind: 'FittableColumnsLayout', components: [
					{kind: 'moon.Input', placeholder: 'Search...', fit: true, oninput: 'searchInputChange'},
		...
		{name: 'popup', kind: 'moon.Popup', modal: true, centered: true, classes: 'list-sample-around-popup', components: [
			{components: [
				{content: 'count:', classes: 'list-sample-around-label'},
				{kind: 'moon.InputDecorator', components: [
					{name: 'countInput', kind: 'moon.Input', style: 'width: 80px', value: 100}
				]}
			]},
			{components: [
				{content: 'rowsPerPage:', classes: 'list-sample-around-label'},
				{kind: 'moon.InputDecorator', components: [
					{name: 'rowsPerPageInput', kind: 'moon.Input', style: 'width: 80px', value: 50}
				]}
			]},
			{components: [
				{content: 'hide divider:', classes: 'list-sample-around-label'},
				{name: 'hideDividerCheckbox', kind: 'moon.Checkbox'}
			]},
			{components: [
				{kind: 'moon.Button', content: 'populate list', classes: 'list-sample-around-populate-button', ontap: 'populateList'}
			]}
		]}
	],
...
enyo.kind({
	...
	components: [
		...
		{name: 'remove', kind: 'moon.IconButton', classes: 'list-sample-around-remove-button', src: 'assets/remove-icon.png', ontap: 'removeTap'}
	],
